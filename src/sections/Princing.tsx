'use client'

import { useEffect, useRef } from 'react'
import { Element } from 'react-scroll'
import clsx from 'clsx'
import Image from 'next/image'
import gsap from 'gsap'
import { ScrollTrigger } from 'gsap/ScrollTrigger'
import { plans } from '../constants'
import Button from '../components/Button'

gsap.registerPlugin(ScrollTrigger)

interface Plan {
  id: string
  logo: string
  title: string
  priceMonthly: number
  priceYearly: number
  caption: string
  icon: string
  features: string[]
}

const Pricing = () => {
  const cardsRef = useRef<HTMLDivElement[]>([])
  const bgRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    const ctx = gsap.context(() => {
      cardsRef.current.forEach((el, index) => {
        gsap.from(el, {
          opacity: 0,
          y: 60,
          duration: 1,
          delay: index * 0.2,
          scrollTrigger: {
            trigger: el,
            start: 'top 85%',
          },
        })
      })

      if (bgRef.current) {
        gsap.to(bgRef.current, {
          yPercent: 20,
          ease: 'none',
          scrollTrigger: {
            trigger: bgRef.current,
            start: 'top bottom',
            end: 'bottom top',
            scrub: true,
          },
        })
      }
    })

    return () => ctx.revert()
  }, [])

  return (
    <section>
      <Element name="pricing">
        <div className="container">
          {/* Header */}
          <div
            className="max-w-960 pricing-head_before relative mx-auto border-l border-r border-s2 bg-s1/50 pb-40 pt-28 max-xl:max-w-4xl max-lg:border-none max-md:pb-32 max-md:pt-16"
            ref={bgRef}
          >
            <h3 className="h3 max-lg:h4 max-md:h5 z-3 relative mx-auto mb-14 max-w-lg text-center text-p4 max-md:mb-11 max-sm:max-w-sm">
              Flexible plans to fit your learning goals
            </h3>

            {/* Parallax Backgrounds */}
            <div className="pricing-bg">
              <Image
                src="/images/bg-outlines.svg"
                width={960}
                height={380}
                alt="outline"
                className="relative z-2"
              />
              <Image
                src="/images/bg-outlines-fill.png"
                width={960}
                height={380}
                alt="outline fill"
                className="absolute inset-0 opacity-5 mix-blend-soft-light"
              />
            </div>
          </div>

          {/* Pricing Cards */}
          <div className="scroll-hide relative z-2 -mt-12 flex items-start max-xl:gap-5 max-xl:overflow-auto max-xl:pt-6">
            {plans.map((plan, index) => (
              <div
                key={plan.id}
                className="pricing-plan_first pricing-plan_last pricing-plan_odd pricing-plan_even relative border-2 p-7 rounded-3xl w-full max-w-[300px] flex-shrink-0"

                ref={(el) => {
                  if (el) cardsRef.current[index] = el
                }}
              >
                {/* Highlight background */}
                {index === 1 && (
                  <div className="g4 absolute h-330 left-0 right-0 top-0 z-1 rounded-tl-3xl rounded-tr-3xl" />
                )}

                {/* Logo */}
                <div
                  className={clsx(
                    'absolute left-0 right-0 z-2 flex items-center justify-center',
                    index === 1 ? '-top-6' : '-top-6 xl:-top-11'
                  )}
                >
                  <Image
                    src={plan.logo}
                    alt={plan.title}
                    width={index === 1 ? 120 : 88}
                    height={index === 1 ? 120 : 88}
                    className="object-contain drop-shadow-2xl"
                  />
                </div>

                {/* Content */}
                <div
                  className={clsx(
                    'relative flex flex-col items-center',
                    index === 1 ? 'pt-24' : 'pt-12'
                  )}
                >
                  <div
                    className={clsx(
                      'small-2 rounded-20 relative z-2 mx-auto mb-6 border-2 px-4 py-1.5 uppercase',
                      index === 1 ? 'border-p3 text-p3' : 'border-p1 text-p1'
                    )}
                  >
                    {plan.title}
                  </div>

                  {/* Price */}
                  <div className="relative z-2 flex items-center justify-center">
                    <div className={clsx('flex items-start text-4xl font-bold', index === 1 ? 'text-p3' : 'text-p4')}>
                         ₦{plan.priceMonthly?.toLocaleString()}
                    </div>
                    
                  </div>
                </div>

                {/* Caption */}
                <div
                  className={clsx(
                    'body-1 relative z-2 mb-10 w-full border-b-s2 pb-9 text-center text-p4',
                    index === 1 && 'border-b'
                  )}
                >
                  {plan.caption}
                </div>

                {/* Features */}
                <ul className="mx-auto space-y-4 xl:px-7">
                  {plan.features.map((feature) => (
                    <li key={feature} className="relative flex items-center gap-5">
                      <Image
                        src="/images/check.png"
                        alt="check"
                        width={40}
                        height={40}
                        className="object-contain"
                      />
                      <p className="flex-1">{feature}</p>
                    </li>
                  ))}
                </ul>

                {/* CTA */}
                <div className="mt-10 flex w-full justify-center">
                <a href='https://forms.gle/GjvtVf7S8Zq54xHfA' target='_blank' rel='noopener noreferrer'>
                      <Button icon={plan.icon}>Get Started</Button>
                </a>
                </div>

                {/* Note */}
                {index === 1 && (
                  <p className="small-compact mt-9 text-center text-p3 before:mx-2.5 before:content-['-'] after:mx-2.5 after:content-['-']">
                    Limited time offer
                  </p>
                )}
              </div>
            ))}
          </div>
        </div>
      </Element>
    </section>
  )
}

export default Pricing
